...
def get_relaxed_energy(atoms, label):
    ...
    traj = Trajectory(f"{label}.traj", 'w', atoms)
...
sub_atoms = slab[(slab.positions[:, 2] < (z_surf - 0.5)) & (slab.positions[:, 2] > (z_sub - 0.5))]
...
third_idx = np.argsort(dists)[2]
third_pos = surf_atoms.positions[third_idx]
